...
const require$$2$2 = require("https");
...
function requireContentType() {
  ...
  var PARAM_REGEXP2 = /; *([!#$%&'*+.^_`|~0-9A-Za-z-]+) *= *("(?:[\u000b\u0020\u0021\u0023-\u005b\u005d-\u007e\u0080-\u00ff]|\\[\u000b\u0020-\u00ff])*"|[!#$%&'*+.^_`|~0-9A-Za-z-]+) */g;
  var TEXT_REGEXP2 = /^[\u000b\u0020-\u007e\u0080-\u00ff]+$/;
  var TOKEN_REGEXP2 = /^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/;
  var QESC_REGEXP2 = /\\([\u000b\u0020-\u00ff])/g;
  var QUOTE_REGEXP2 = /([\\"])/g;
  ...
  function format2(obj) {
    ...
    if (parameters && typeof parameters === "object") {
      ...
      for (var i = 0; i < params.length; i++) {
        param3 = params[i];
        if (!TOKEN_REGEXP2.test(param3)) {
          throw new TypeError("invalid parameter name");
        }
        string += "; " + param3 + "=" + qstring2(parameters[param3]);
      }
    }
    return string;
  }
  function parse2(string) {
    ...
    if (index2 !== -1) {
      ...
      var match2;
      var value;
      PARAM_REGEXP2.lastIndex = index2;
      while (match2 = PARAM_REGEXP2.exec(header3)) {
        ...
        if (value.charCodeAt(0) === 34) {
          ...
          if (value.indexOf("\\") !== -1) {
            value = value.replace(QESC_REGEXP2, "$1");
          }
        }
  ...
  function qstring2(val) {
    var str = String(val);
    if (TOKEN_REGEXP2.test(str)) {
      return str;
    }
    if (str.length > 0 && !TEXT_REGEXP2.test(str)) {
      throw new TypeError("invalid parameter value");
    }
    return '"' + str.replace(QUOTE_REGEXP2, "\\$1") + '"';
  }
...
function requireStreams() {
  ...
  var Buffer2 = require$$0$5.Buffer, Transform = require$$1$3.Transform;
...
function requireRawBody() {
  ...
  var createError2 = requireHttpErrors();
...
function onFinished$6(msg, listener2) {
  if (isFinished$2(msg) !== false) {
    defer$2(listener2, null, msg);
    return msg;
  }
  attachListener$1(msg, wrap$1(listener2));
  return msg;
}
...
function attachListener$1(msg, listener2) {
...
function patchAssignSocket$1(res2, callback) {
...
function requireRead() {
  ...
  hasRequiredRead = 1;
  var createError2 = requireHttpErrors();
  var destroy2 = requireDestroy();
  var getBody = requireRawBody();
  var iconv = requireLib$1();
  var onFinished2 = onFinishedExports$1;
  var unpipe2 = requireUnpipe();
  var zlib = require$$3$2;
  read_1 = read;
...
function requireTypeIs() {
  ...
  function typeis2(value, types_) {
    ...
    var type3;
    for (i = 0; i < types.length; i++) {
      if (mimeMatch(normalize2(type3 = types[i]), val)) {
  ...
  function normalize2(type3) {
    ...
    return type3.indexOf("/") === -1 ? mime2.lookup(type3) : type3;
  }
...
function requireJson() {
  ...
  hasRequiredJson = 1;
  var bytes2 = requireBytes();
  var contentType3 = requireContentType();
  var createError2 = requireHttpErrors();
  var debug2 = requireSrc()("body-parser:json");
  var read = requireRead();
  var typeis2 = requireTypeIs();
  json_1 = json2;
...
function requireStringify() {
  ...
  var sentinel = {};
  var stringify2 = function stringify3(object, prefix, generateArrayPrefix, commaRoundTrip, strictNullHandling, skipNulls, encoder, filter, sort, allowDots, serializeDate, format2, formatter, encodeValuesOnly, charset3, sideChannel2) {
    var obj = object;
    ...
    if (obj === null) {
      if (strictNullHandling) {
        return encoder && !encodeValuesOnly ? encoder(prefix, defaults.encoder, charset3, "key", format2) : prefix;
      }
      obj = "";
    }
    if (isNonNullishPrimitive(obj) || utils2.isBuffer(obj)) {
      if (encoder) {
        var keyValue = encodeValuesOnly ? prefix : encoder(prefix, defaults.encoder, charset3, "key", format2);
        if (generateArrayPrefix === "comma" && encodeValuesOnly) {
          var valuesArray = split.call(String(obj), ",");
          var valuesJoined = "";
          for (var i = 0; i < valuesArray.length; ++i) {
            valuesJoined += (i === 0 ? "" : ",") + formatter(encoder(valuesArray[i], defaults.encoder, charset3, "value", format2));
          }
          return [formatter(keyValue) + (commaRoundTrip && isArray(obj) && valuesArray.length === 1 ? "[]" : "") + "=" + valuesJoined];
        }
        return [formatter(keyValue) + "=" + formatter(encoder(obj, defaults.encoder, charset3, "value", format2))];
      }
  ...
  var normalizeStringifyOptions = function normalizeStringifyOptions2(opts) {
    ...
    var charset3 = opts.charset || defaults.charset;
 ...
 * MIT Licensed
 */
var debug$9 = srcExports$3("finalhandler");
...
function finalhandler(req2, res2, options) {
  ...
  return function(err) {
    ...
    if (err && onerror) {
      defer$1(onerror, err, req2, res2);
    }
    if (headersSent$2(res2)) {
...
function send$3(req2, res2, status3, headers, message) {
  ...
  if (isFinished$1(req2)) {
    write();
    return;
  }
  unpipe(req2);
  onFinished$5(req2, write);
  req2.resume();
}
...
var debug$7 = srcExports$2("express:router:layer");
 ...
 * MIT Licensed
 */
var debug$6 = srcExports$2("express:router:route");
...
var debug$5 = srcExports$2("express:router");
var deprecate$4 = depd_1("express");
 ...
 * MIT Licensed
 */
var debug$4 = srcExports$2("express:view");
...
var safeBufferExports$2 = safeBuffer$2.exports;
...
function contentDisposition$1(filename, options) {
  ...
  return format$1(new ContentDisposition(type3, params));
}
...
var debug$2 = srcExports$1("send");
var deprecate$3 = depd_1("send");
...
function append(header3, field) {
  ...
  var val = header3;
  var vals = parse$2(header3.toLowerCase());
...
res.send = function send3(body) {
  ...
  if (chunk !== void 0) {
    if (Buffer$2.isBuffer(chunk)) {
      ...
      len = Buffer$2.byteLength(chunk, encoding3);
    } else {
...
res.jsonp = function jsonp(obj) {
  ...
  if (arguments.length === 2) {
    if (typeof arguments[1] === "number") {
      deprecate$1("res.jsonp(obj, status): Use res.status(status).jsonp(obj) instead");
      this.statusCode = arguments[1];
    } else {
...
(function(module, exports) {
  ...
  function createApplication() {
    ...
    mixin2(app, EventEmitter.prototype, false);
...
basicAuth.exports.parse = parse$1;
var CREDENTIALS_REGEXP = /^ *(?:[Bb][Aa][Ss][Ii][Cc]) +([A-Za-z0-9._~+/-]+=*) *$/;
var USER_PASS_REGEXP = /^([^:]*):(.*)$/;
function auth$1(req2) {
  if (!req2) {
    throw new TypeError("argument req is required");
  }
  ...
  var header3 = getAuthorization(req2);
  return parse$1(header3);
}
function decodeBase64(str) {
  return Buffer$1.from(str, "base64").toString();
}
function getAuthorization(req2) {
  if (!req2.headers || typeof req2.headers !== "object") {
    throw new TypeError("argument req is required to have headers property");
  }
  return req2.headers.authorization;
}
function parse$1(string) {
  if (typeof string !== "string") {
    return void 0;
  }
  var match2 = CREDENTIALS_REGEXP.exec(string);
  if (!match2) {
    return void 0;
  }
  var userPass = USER_PASS_REGEXP.exec(decodeBase64(match2[1]));
  if (!userPass) {
    return void 0;
  }
...
function onHeaders$1(res2, listener2) {
  if (!res2) {
  ...
  res2.writeHead = createWriteHead(res2.writeHead, listener2);
}
function setHeadersFromArray(res2, headers) {
  for (var i = 0; i < headers.length; i++) {
    res2.setHeader(headers[i][0], headers[i][1]);
  }
}
function setHeadersFromObject(res2, headers) {
  var keys = Object.keys(headers);
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    if (k)
      res2.setHeader(k, headers[k]);
  }
}
...
function morgan(format2, options) {
  ...
  var stream2 = opts.stream || process.stdout;
  if (buffer) {
    deprecate("buffer option");
    var interval = typeof buffer !== "number" ? DEFAULT_BUFFER_DURATION : buffer;
    stream2 = createBufferStream(stream2, interval);
  }
  return function logger(req2, res2, next) {
    req2._startAt = void 0;
    req2._startTime = void 0;
    req2._remoteAddress = getip(req2);
    res2._startAt = void 0;
    res2._startTime = void 0;
    recordStartTime.call(req2);
    function logRequest() {
      if (skip !== false && skip(req2, res2)) {
        debug("skip request");
        return;
      }
      var line = formatLine(morgan, req2, res2);
      if (line == null) {
        debug("skip line");
        return;
      }
    ...
    if (immediate) {
      ...
      onHeaders(res2, recordStartTime);
      onFinished(res2, logRequest);
    }
    next();
  };
}
...
morgan.format("dev", function developmentFormatLine(tokens, req2, res2) {
  var status3 = headersSent(res2) ? res2.statusCode : void 0;
  var color = status3 >= 500 ? 31 : status3 >= 400 ? 33 : status3 >= 300 ? 36 : status3 >= 200 ? 32 : 0;
  ...
  return fn(tokens, req2, res2);
});
morgan.token("url", function getUrlToken(req2) {
  return req2.originalUrl || req2.url;
});
morgan.token("method", function getMethodToken(req2) {
  return req2.method;
});
morgan.token("response-time", function getResponseTimeToken(req2, res2, digits) {
  if (!req2._startAt || !res2._startAt) {
    return;
  }
  var ms2 = (res2._startAt[0] - req2._startAt[0]) * 1e3 + (res2._startAt[1] - req2._startAt[1]) * 1e-6;
  return ms2.toFixed(digits === void 0 ? 3 : digits);
});
morgan.token("total-time", function getTotalTimeToken(req2, res2, digits) {
  if (!req2._startAt || !res2._startAt) {
    return;
  }
  var elapsed = process.hrtime(req2._startAt);
  var ms2 = elapsed[0] * 1e3 + elapsed[1] * 1e-6;
  return ms2.toFixed(digits === void 0 ? 3 : digits);
});
morgan.token("date", function getDateToken(req2, res2, format2) {
...
morgan.token("status", function getStatusToken(req2, res2) {
  return headersSent(res2) ? String(res2.statusCode) : void 0;
});
morgan.token("referrer", function getReferrerToken(req2) {
  return req2.headers.referer || req2.headers.referrer;
});
morgan.token("remote-addr", getip);
morgan.token("remote-user", function getRemoteUserToken(req2) {
  var credentials = auth(req2);
  return credentials ? credentials.name : void 0;
});
morgan.token("http-version", function getHttpVersionToken(req2) {
  return req2.httpVersionMajor + "." + req2.httpVersionMinor;
});
morgan.token("user-agent", function getUserAgentToken(req2) {
  return req2.headers["user-agent"];
});
morgan.token("req", function getRequestToken(req2, res2, field) {
  var header3 = req2.headers[field.toLowerCase()];
  return Array.isArray(header3) ? header3.join(", ") : header3;
});
morgan.token("res", function getResponseHeader(req2, res2, field) {
  if (!headersSent(res2)) {
    return void 0;
  }
  var header3 = res2.getHeader(field);
  return Array.isArray(header3) ? header3.join(", ") : header3;
});
...
function createBufferStream(stream2, interval) {
  var buf = [];
  var timer = null;
  function flush() {
    timer = null;
    stream2.write(buf.join(""));
    buf.length = 0;
  }
  function write(str) {
    if (timer === null) {
...
function getip(req2) {
  return req2.ip || req2._remoteAddress || req2.connection && req2.connection.remoteAddress || void 0;
}
...
const morgan$1 = /* @__PURE__ */ getDefaultExportFromCjs(morganExports);
var lib = { exports: {} };
/*
object-assign
(c) Sindre Sorhus
...
const addProduct = async ({ code, name, brand, category, stock, profit_margin_mayor, profit_margin_menor, discount, cost, sale_mayor, sale_menor }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "INSERT INTO products (code, name, brand, category, stock, profit_margin_mayor, profit_margin_menor, discount,cost, sale_mayor, sale_menor) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.run(query3, [code, name, brand, category, stock, profit_margin_mayor, profit_margin_menor, discount, cost, sale_mayor, sale_menor], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Product ${name} added successfully`));
      }
    });
  });
};
const updateProduct = async (id_product, { code, name, brand, category, stock, profit_margin_mayor, profit_margin_menor, discount, cost, sale_mayor, sale_menor }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "UPDATE products SET code = ?, name = ?, brand = ?, category = ?, stock = ?, profit_margin_mayor = ?, profit_margin_menor = ?, discount = ?,cost = ?, sale_mayor = ?, sale_menor = ? WHERE id_product = ?";
    db.run(query3, [code, name, brand, category, stock, profit_margin_mayor, profit_margin_menor, discount, cost, sale_mayor, sale_menor], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Product with id ${id_product} updated successfully`, code, name, brand, cost, sale_mayor, sale_menor));
      }
    });
  });
};
const deleteProduct = async (id_product) => {
  return new Promise((resolve3, reject) => {
    const query3 = "DELETE FROM products WHERE id_product = ?";
    db.run(query3, [id_product], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Product with id ${id_product} deleted successfully`));
      }
    });
  });
};
router.get("/api/products", async (req2, res2) => {
  req2.setTimeout(0);
  try {
    ...
    res2.status(500).json({ error: "Error fetching products" });
  }
});
router.post("/api/products", async (req2, res2) => {
  try {
    await addProduct(req2.body);
    res2.sendStatus(201);
  } catch (error2) {
    console.error("Error al agregar el producto:", error2);
    res2.status(500).json({ error: "Error al agregar el producto" });
  }
});
router.put("/api/products/:id_product", async (req2, res2) => {
  const { id_product } = req2.params;
  const { code, name, brand, cost, sale_mayor, sale_menor } = req2.body;
  try {
    await updateProduct(id_product, { code, name, brand, cost, sale_mayor, sale_menor });
    res2.sendStatus(204);
  } catch (error2) {
    console.error("Error al actualizar el producto:", error2);
    res2.status(500).json({ error: "Error al actualizar el producto" });
  }
});
router.delete("/api/products/:id_product", async (req2, res2) => {
  const { id_product } = req2.params;
  try {
    deleteProduct(id_product);
    res2.sendStatus(204);
  } catch (error2) {
    console.error("Error al eliminar el producto:", error2);
    res2.status(500).json({ error: "Error al eliminar el producto" });
  }
});
...
const addCustomer = async ({ razon_social, cuit, condicion_iva, email, phone, address, city, saldo }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "INSERT INTO customers (razon_social, cuit, condicion_iva, email, phone, address, city, saldo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    db.run(query3, [razon_social, cuit, condicion_iva, email, phone, address, city, saldo], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Customer ${razon_social} added successfully`));
      }
    });
  });
};
const deleteCustomer = async (id) => {
  return new Promise((resolve3, reject) => {
    const query3 = "DELETE FROM customers WHERE id = ?";
    db.run(query3, [id], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Customer with id ${id} deleted successfully`));
      }
    });
  });
};
const updateCustomer = async (id, { razon_social, cuit, condicion_iva, email, phone, address, city, saldo }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "UPDATE customers SET razon_social = ?, cuit = ?, condicion_iva = ?, email = ?, phone = ?, address = ?, city = ?, saldo = ? WHERE id = ?";
    db.run(query3, [razon_social, cuit, condicion_iva, email, phone, address, city, saldo, id], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Customer with id ${id} updated successfully`, razon_social, cuit, condicion_iva, email, phone, address, city, saldo));
      }
    });
  });
};
router.get("/api/customers", async (req2, res2) => {
  req2.setTimeout(0);
  try {
    ...
    res2.status(500).json({ error: "Error fetching customers" });
  }
});
router.post("/api/customers", (req2, res2) => {
  const { razon_social, cuit, condicion_iva, email, phone, address, city, saldo } = req2.body;
  try {
    addCustomer({ razon_social, cuit, condicion_iva, email, phone, address, city, saldo });
    res2.sendStatus(201);
  } catch (error2) {
    console.error("Error adding customer:", error2);
    res2.status(500).json({ error: "Error adding customer" });
  }
});
router.put("/api/customers/:id", (req2, res2) => {
  ...
  try {
    ...
    console.error("Error updating customer:", error2);
    res2.status(500).json({ error: "Error updating customer" });
  }
});
router.delete("/api/customers/:id", (req2, res2) => {
  const { id } = req2.params;
  try {
    deleteCustomer(id);
    res2.sendStatus(204);
    ...
    res2.status(500).json({ error: "Error deleting customer" });
  }
});
const customer = router;
const fetchCaja = async () => {
  return new Promise((resolve3, reject) => {
    const query3 = "SELECT * FROM caja";
    db.all(query3, [], (err, rows) => {
      if (err) {
        reject(err);
      } else {
...
const movimientoCaja$1 = async ({ tipo, importe, motivo, equipo, id_customer, saldo }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "INSERT INTO caja (tipo, importe, motivo, equipo, id_customer, saldo) VALUES (?, ?, ?, ?, ?, ?)";
    db.run(query3, [tipo, importe, motivo, equipo, id_customer, saldo], (err) => {
      if (err) {
        reject(err);
      } else {
        resolve3(console.log(`Movimiento de ${tipo} realizado correctamente`));
      }
    });
  });
};
router.get("/api/caja", async (req2, res2) => {
  req2.setTimeout(0);
  try {
    ...
    console.error("Error fetching movements:", error2);
    res2.status(500).json({ error: "Error fetching movements" });
  }
});
router.post("/api/caja", async (req2, res2) => {
  console.log(req2.body);
  try {
    await movimientoCaja$1(req2.body);
    console.log("Movimiento de caja realizado correctamente");
    res2.sendStatus(201);
  } catch (error2) {
    console.error("Error al realizar el movimiento:", error2);
    res2.status(500).json({ error: "Error al realizar el movimiento" });
  }
});
const caja = router;
const movimientoCaja = async ({ tipo, importe, motivo, equipo, id_customer, saldo }) => {
  return new Promise((resolve3, reject) => {
    const query3 = "INSERT INTO caja (tipo, importe, motivo, equipo, id_customer, saldo) VALUES (?, ?, ?, ?, ?, ?)";
    db.run(query3, [tipo, importe, motivo, equipo, id_customer, saldo], (err) => {
      if (err) {
...
router.get("/api/pedidos", async (req2, res2) => {
  req2.setTimeout(0);
});
router.post("/api/pedidos", async (req2, res2) => {
  console.log(req2.body);
  try {
    await movimientoCaja(req2.body);
    console.log("Movimiento de caja realizado correctamente");
    res2.sendStatus(201);
  } catch (error2) {
    console.error("Error al realizar el movimiento:", error2);
...
function EventSource(url2, eventSourceInitDict) {
  ...
  function connect() {
    ...
    if (headers) {
      ...
      for (var i in reqHeaders) {
        var header3 = reqHeaders[i];
        if (header3) {
          options.headers[i] = header3;
        }
      }
    }
    ...
    req2 = (isSecure ? https : http).request(options, function(res2) {
      self2.connectionInProgress = false;
      if (res2.statusCode === 500 || res2.statusCode === 502 || res2.statusCode === 503 || res2.statusCode === 504) {
        _emit("error", new Event("error", { status: res2.statusCode, message: res2.statusMessage }));
        onConnectionClosed();
        return;
      }
      if (res2.statusCode === 301 || res2.statusCode === 302 || res2.statusCode === 307) {
        var location2 = res2.headers.location;
        if (!location2) {
          _emit("error", new Event("error", { status: res2.statusCode, message: res2.statusMessage }));
          return;
        }
        var prevOrigin = new URL(url2).origin;
      ...
      res2.on("close", function() {
        res2.removeAllListeners("close");
        res2.removeAllListeners("end");
        onConnectionClosed();
      });
  ...
  this._close = function() {
    ...
    if (req2.abort)
      req2.abort();
    if (req2.xhr && req2.xhr.abort)
      req2.xhr.abort();
  };
  function parseEventStreamLine(buf, pos, fieldLength, lineLength) {
    if (lineLength === 0) {
      if (data.length > 0) {
        var type3 = eventName || "message";
        _emit(type3, new MessageEvent(type3, {
          data: data.slice(0, -1),
...
["open", "error", "message"].forEach(function(method) {
  Object.defineProperty(EventSource.prototype, "on" + method, {
     ...
     */
    get: function get() {
      var listener2 = this.listeners(method)[0];
      return listener2 ? listener2._listener ? listener2._listener : listener2 : void 0;
    },
    /**
     ...
     * @api private
     */
    set: function set(listener2) {
      this.removeAllListeners(method);
      this.addEventListener(method, listener2);
    }
  });
});
...
EventSource.prototype.addEventListener = function addEventListener(type3, listener2) {
  if (typeof listener2 === "function") {
    listener2._listener = listener2;
    this.on(type3, listener2);
  }
};
...
EventSource.prototype.removeEventListener = function removeEventListener(type3, listener2) {
  if (typeof listener2 === "function") {
    listener2._listener = void 0;
    this.removeListener(type3, listener2);
  }
};
function Event(type3, optionalProperties) {
  Object.defineProperty(this, "type", { writable: false, value: type3, enumerable: true });
...
function MessageEvent(type3, eventInitDict) {
  Object.defineProperty(this, "type", { writable: false, value: type3, enumerable: true });
...
const EventSource$1 = /* @__PURE__ */ getDefaultExportFromCjs(eventsource);
const es = new EventSource$1("http://localhost:3000/events");
...
es.onmessage = (event) => {
  ...
  if (message.type === "addProduct") {
    axios.get("http://localhost:3000/api/products").then(async (response2) => {
      const products = response2.data;
      const oldProducts = await fetchProducts();
      const newProducts = products.filter((product2) => !oldProducts.some((oldProduct) => oldProduct.id_product === product2.id_product));
      if (newProducts.length > 0) {
        ...
        const values = newProducts.flatMap((product2) => [product2.code, product2.name, product2.brand, product2.category, product2.stock, product2.discount, product2.cost, product2.sale_mayor, product2.sale_menor, product2.updated_at]);
...
appExpress.use(express$1.json());
appExpress.use(bodyParser.json());
appExpress.use(morgan$1("dev"));
appExpress.use((error2, _req, res2, _next) => {
  console.error("Error no manejado:", error2);
  res2.status(500).send("Error interno del servidor");
});
...
appExpress.use(express$1.static(path$3.join(__dirname, "../public")));
...
process.env.VITE_PUBLIC = electron.app.isPackaged ? process.env.DIST : path$3.join(process.env.DIST, "../public");
let win;
...
function createWindow() {
  win = new electron.BrowserWindow({
    ...
    webPreferences: {
      preload: path$3.join(__dirname, "../dist-electron/preload.js"),
